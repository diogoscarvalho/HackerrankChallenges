    ...
    */
    internal class Program
    {
        static void Main(string[] args)
        {
            ...
            for (int i = 0; i < q; i++)
            {
                queries.Add(Console.ReadLine().TrimEnd().Split(' ').AsEnumerable().Select(queriesTemp => Convert.ToInt32(queriesTemp)).ToList());
            }

            List<int> result = DynamicArray.Queries(n, queries);
